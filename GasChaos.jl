...
# ╔═╡ edd88bd9-33ea-4d1c-a0ab-a220de235c1e
using Distributed 
...
function barenblatt(x, t, m, d=1, Γ=1) #0.2
...
# ╔═╡ 256b3b5e-81c2-4b81-9c1d-b313296afc45
function barenblattProf(x,t,m)
    tx=t^(-1.0/(m+1.0))
    xx=x*tx
    xx=xx*xx
...
# ╔═╡ e11520ec-65fb-457f-b44c-83d6a5289c64
uu=(-1:0.1:1)

# ╔═╡ 8d6ea261-4031-43ba-99a9-3b184816348f
tom = map(x->barenblatt(x,0.01,2), uu)

# ╔═╡ 8ce0cba5-4c4b-4c1d-9cff-d9656a46d454
prof = map(x->barenblattProf(x,0.01,2), uu)

# ╔═╡ 0aff086f-7404-4b85-beec-ace7af4851f0
tom[5] , prof[5]
...
function main(;n = 50 , m = 3, Plotter = nothing, verbose = false, 						 unknown_storage = :sparse, tend = 0.1, tstep = 0.0001, d = 1, L = 1)
...
sys, grid, tsol, error = main(tend = 0.01);

# ╔═╡ bffefc30-70fe-4a1b-bf8c-686758e70c4e
error

# ╔═╡ 82fd612a-ea5b-49cb-b5d9-ba183622fc8e
begin
	errorVector=Float64[]

@distributed for i in 10:150
	s1, g1, t1, error = main(n=i, tend = 0.01)
	push!(errorVector, error)
...
# ╔═╡ ecbdf6ac-aec3-4d76-a2f8-7eed1d750135
errorVector

# ╔═╡ 34c032fc-1e68-48a8-8393-6aa7611a7549
plot(errorVector,xlabel="Grid size", ylabel="Error Val")

# ╔═╡ 311478c4-cf77-479a-81db-a30c5547fd5c
findall(x->x==minimum(errorVector), errorVector) #Inedx of the Error i.e the griddensity number
...
begin
	...
	scalarplot!(vis[3, 1], sys, tsol, aspect=100, title="Spacetime diagram", levels=20)	

	reveal(vis)   #= somehow the "sys" gives max(y_value) = 0.10 here, but should be 0.01 if it has to be compared for error values, bcz of the use of barenblat(t=0.01) vs the last vector of tsol[1,:,end] as a comparison to cal. error =#
end

# ╔═╡ 91d3364f-8d63-49d4-b0df-b1f8ad1c1876


# ╔═╡ 7c4f0548-fb06-4960-9178-7a0e9dc4e8b8
tsol[1, :, timestep]
