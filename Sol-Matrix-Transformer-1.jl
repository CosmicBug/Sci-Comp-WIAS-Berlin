...
# ╔═╡ 9ec839ae-63e9-44d2-804b-16bbf65a49d8
time_sol_2d

# ╔═╡ 7696a878-8e17-4a73-b23c-08521c64fb7c
time_sol_2d[1][81:100]

# ╔═╡ ab0a9eb7-2989-436a-84ba-1cc325a46940
time_sol_2d[1][:]

# ╔═╡ 3e0f1b81-27aa-4cd3-a07d-329b07add8b0
begin 
plotMat=zeros(length(time_sol_2d.t),length(time_sol_1d.u[1]),length(time_sol_1d.u[1])) #(192 , 41 , 41) for n=20
...
begin 
	surface(plotMat[1,:,:],markeralpha = 0)
	surface!(plotMat[50,:,:],markeralpha = 0.9)
	surface!(plotMat[100,:,:],markeralpha = 0.9)
	surface!(plotMat[150,:,:],markeralpha = 0.9)
	surface!(plotMat[191,:,:],markeralpha = 0.9)
end

# ╔═╡ a896264c-b0f5-4085-8f94-d017544cd7ed
t = @bind t Slider(1:length(time_sol_2d.t), show_value = true)

# ╔═╡ f99870fa-e9b8-4b6b-a21f-b031ad3790bd
surface(plotMat[t,:,:],markeralpha = 0.9)

# ╔═╡ ab91a182-0c21-4b17-b599-bfb413a57055
md"## DOESNT WORK ⇓" 

# ╔═╡ b39fbeb2-a579-4993-ba21-743a99306721
begin
for i in 1:192
	surface!(plotMat[i,:,:])   #where is the plot my dude! 
end
end

# ╔═╡ 7bd838fd-9b18-4ed7-9ee4-d407cc13f037
