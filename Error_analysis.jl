...
# ╔═╡ 4cebd5c1-d1b8-49d9-a712-bd25927adea3
begin 
	#Extention on Tomasz code 
...
# ╔═╡ 655605ce-8609-4054-9c20-48b5bc83e8c4
function barenblatt_iniv(x; t = 0.001, M = 2 , dims = 1) # Method for 1D grid
...
# ╔═╡ 54d73a3b-b402-4fee-b6ae-11206e890edd
function barenblatt_iniv(x, y; t = 0.001, M = 2 , dims = 2) # Method for 2D grid
...
# ╔═╡ 6baafaa3-f729-4ce5-94b4-f641d1e06f7a
function barenblatt_endv(x; t = 0.01 , M = 2 , dims = 1) # Method for 1D grid
...
# ╔═╡ c7f57c8f-3864-4ea2-933d-8ac3c780c9b2
function barenblatt_endv(x, y; t = 0.01, M = 2 , dims = 2) # Method for 2D grid
...
# ╔═╡ 5e476a0a-9a28-4837-aa19-443d3ee41353
...
# ╔═╡ 0c9aeb71-c06a-4a06-918a-43709442aa86
...
# ╔═╡ b21558b4-0ddc-4ce3-b80e-df9d56894c00
...
# ╔═╡ ab8cdd50-38ae-4ddf-b8b0-96f1b2e927a3
begin 

#=function barenblatt(x,t,m) #so the fucntion is fine
    tx=t^(-1.0/(m+1.0))
    xx=x*tx
    xx=xx*xx
...
function run_vfvm(;n=20,m=2,t0=0.001, tend=0.01,tstep=1.0e-6,unknown_storage=:dense)
    ...
    X=collect(-1:h:1)
    grid=simplexgrid(X) #VoronoiFVM.Grid(X)
...
# ╔═╡ 54f6b3f2-ae8e-479c-99a3-c3bec304b4f6
begin

	
function vfvm_2D(;n=20,m=2,t0=0.001, tend=0.01,tstep=1.0e-6,unknown_storage=:dense)

	h=1.0/convert(Float64,n/2)
    X=collect(-1:h:1)
    grid=simplexgrid(X, X) #VoronoiFVM.Grid(X)
    ...
    inival=unknowns(sys)
    inival[1,:].= map(barenblatt_iniv, grid) #map(x->barenblatt(x,t0,m),X)
    sol=VoronoiFVM.solve(sys;inival,times=(t0,tend),Δt=tstep,Δu_opt=0.01,Δt_min=tstep,store_all=true,log=true)
    err=norm(sol[1,:,end]- (map(barenblatt_endv, grid)))
    sol,sys,err
end

end

# ╔═╡ 9978d31b-d2d5-4f82-ac93-4b2e64765451
function create_error_array_voronoi2(dim, limit, step = 10)
    error_array = []
...
# ╔═╡ 0762ec9c-bbec-4278-86c6-2576c527668f
err_vor2 = create_error_array_voronoi2(2,60) #2D Voronoi

# ╔═╡ ca0765bf-09c6-4164-86ac-39e78c2f1761
plot(err_vor2)

# ╔═╡ fc4c5956-8fce-4441-b6ae-81632fa23c69
function create_error_array_voronoi(dim, limit, step = 10)
    error_array = []
    for i in collect(10:step:limit)
        #grid = create_grid(i,dim)
        #system = create_system(grid)
...
# ╔═╡ 70a808ba-bd79-4292-bee8-925d082df0cd
err_vor = create_error_array_voronoi(1,200)

# ╔═╡ a65ce7a0-ac8a-45a4-aae4-bde7d252c6a9
plot(err_vor) #1D Voronoi

# ╔═╡ 05941540-c6d4-11ec-354c-4fbbc1c810d8
...
# ╔═╡ a5531692-7a88-48e2-b513-36551bf66985
#err2d = create_error_array(2,40) 

# ╔═╡ a398a646-7ba2-4758-993f-e505a564f915
plot(err2d)

# ╔═╡ 66cfc7d2-c7ec-4658-bab6-1d292ee0e21e
#################### testing \/
